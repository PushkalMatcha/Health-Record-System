...
app.get('/', async (req, res) => {
    const users = await prisma_1.prisma.user.findMany({ take: 5 });
...
app.post('/api/auth/login', async (req, res) => {
    try {
        ...
        if (!email || (!password && !dob))
            return res.status(400).json({ error: 'email and password or dob required' });
        const user = await prisma_1.prisma.user.findUnique({ where: { email }, include: { volunteer: true } });
        if (!user)
...
app.get('/api/admin/stats', authMiddleware, isAdmin, async (req, res) => {
    try {
        const [usersCount, volunteersCount, patientsCount, visitsCount] = await Promise.all([
            ...
            prisma_1.prisma.volunteer.count(),
        ...
        res.json({
            ...
            visitsByStatus: visitsByStatus.reduce((acc, { status, _count }) => ({
                ...acc,
...
app.get('/api/volunteers', async (req, res) => {
    try {
        const volunteers = await prisma_1.prisma.volunteer.findMany({
            include: {
                user: {
                    select: {
                        email: true
...
app.post('/api/volunteers', async (req, res) => {
    try {
        const { name, email, phone, village, specialization, dob, address, emergencyContact, } = req.body;
        if (!email || !name)
        ...
        const hashed = await bcryptjs_1.default.hash(randomPass, 10);
        const user = await prisma_1.prisma.user.create({
            data: {
                email,
                password: hashed,
        ...
        const volunteer = await prisma_1.prisma.volunteer.create({
            data: {
                userId: user.id,
                firstName: firstName,
                lastName: lastName,
...
app.get('/api/volunteers/:id', async (req, res) => {
    try {
        ...
        const transformedVolunteer = {
            ...
            emergencyContact: volunteer.emergencyContact || null
        };
...
app.put('/api/volunteers/:id', authMiddleware, async (req, res) => {
    try {
        ...
        const transformedVolunteer = {
            ...
            email: updatedVolunteer.user.email,
            phone: updatedVolunteer.phone,
            village: updatedVolunteer.village || '',
            ...
            emergencyContact: updatedVolunteer.emergencyContact
        };
...
app.get('/api/patients', authMiddleware, async (req, res) => {
    try {
        ...
        const transformedPatients = patients.map((p) => ({
            ...
            gender: p.medicalHistory?.gender || 'Other',
            village: p.address?.split(',')[1]?.trim() || 'Unknown',
            phoneNumber: p.phone,
            bloodGroup: p.medicalHistory?.bloodGroup || 'Unknown',
            lastVisit: p.updatedAt.toISOString().split('T')[0],
            priority: p.medicalHistory?.priority || 'Medium',
            chronicConditions: p.medicalHistory?.chronicConditions || [],
            allergies: p.medicalHistory?.allergies || [],
            emergencyContact: p.emergencyContact?.name || 'Unknown',
            emergencyPhone: p.emergencyContact?.phone || 'Unknown',
...
app.get('/api/patients/:id', authMiddleware, async (req, res) => {
    try {
        ...
        const transformedPatient = {
            ...
            gender: patient.medicalHistory?.gender || 'Other',
            ...
            priority: patient.medicalHistory?.priority || 'Medium',
            chronicConditions: patient.medicalHistory?.chronicConditions || [],
            allergies: patient.medicalHistory?.allergies || [],
            emergencyContact: patient.emergencyContact?.name || 'Unknown',
            emergencyPhone: patient.emergencyContact?.phone || 'Unknown',
